...
module EtradeJanitor.Repos.Yahoo.PaperHistorySpec
  ( spec
  )
where
...
testDay = Calendar.fromGregorian 2019 12 30
...
testTickerNhy dx = Types.Ticker 1 "NHY" 1 dx
  ...
  describe "Yahoo Csv" $ do
    ...
    context "when ticker is EQNR" $ do
      it ("csv path name should be " ++ testCsvPath) $ do
        result <- runReaderT
          (Types.runApp $ PaperHistory.csvPath (testTicker testDay))
          testEnv
        shouldBe result testCsvPath
    context "when ticker is EQNR" $ do
      it "number of stock prices should be 9" $ do
        result <- runReaderT
          (Types.runApp $ PaperHistory.fetchStockPrices (testTicker testDay))
          testEnv
    ...
    context "when ticker is EQNR and contains 3 faulty data" $ do
      it "number of stock prices should be 12" $ do
        ...
        result <- runReaderT
          (Types.runApp $ PaperHistory.fetchStockPrices (testTicker curDate))
          testEnv
    ...
    context "when ticker is NHY and contains only one day" $ do
      it "number of stock prices should be 1" $ do
        ...
        result <- runReaderT
          (Types.runApp $ PaperHistory.fetchStockPrices (testTickerNhy curDate))
          testEnv
        shouldBe (length result) 1
    context
        "when ticker is NHY and contains only one day and ticker date greater than csv date"
      $ do
          it "number of stock prices should be 0" $ do
            let curDate = Calendar.fromGregorian 2020 04 22
            result <- runReaderT
              ( Types.runApp
              $ PaperHistory.fetchStockPrices (testTickerNhy curDate)
              )
              testEnv
